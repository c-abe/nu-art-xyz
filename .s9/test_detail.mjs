...

{
  ...
  // BASEかんたん決済が通ったので購入ボタンを出している（SHOP.live = true）
  const buy = doc.getElementById('dBuy');
  t('購入ボタンが出ている', buy.style.display !== 'none');
  t('購入ボタンの行き先がBASEの商品', /shopselect\.net\/items\/\d+$/.test(buy.href), buy.href);
  t('購入ボタンに値段が入っている', /¥[\d,]+/.test(buy.textContent), buy.textContent);
}
